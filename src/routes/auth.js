...
router.post('/register', async (req, res) => {
  ...
  } catch (err) { console.error(err); res.status(500).json({ error: 'Erro ao criar conta' }) }
})
...
router.post('/login', async (req, res) => {
  ...
  try {
    ...
    if (!user || !user.password_hash) return res.status(401).json({ error: 'Email ou senha inválidos' })
...
router.get('/me', require('../middleware/auth'), async (req, res) => {
  try {
    const result = await pool.query(
      'SELECT id, name, email, avatar_url, points, level, created_at FROM users WHERE id = $1', [req.userId]
    )
    res.json(result.rows[0])
  } catch { res.status(500).json({ error: 'Erro ao buscar usuário' }) }
})
